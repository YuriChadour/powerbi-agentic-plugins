...
## Post-Update BOM Cleanup

After making edits to TMDL or related JSON files, **always run BOM removal** to ensure UTF-8 encoding without BOM (Byte Order Mark). This prevents encoding issues when files are processed by Power BI or other tools.

> **Why this matters:** Power BI and Fabric will reject files with UTF-8 BOM encoding with the error: `Cannot read 'definition/tables/...'. Only text with UTF8 encoding without BOM (byte order marks) is supported. Detected BOM: 'UTF-8'`. The cleanup scripts force a reliable re-save that removes BOM.

### Automated Cleanup Scripts

Three generic scripts are available in `scripts/`:

#### PowerShell: `remove_all_boms.ps1` (Recommended)

Removes UTF-8 BOM from all files matching a pattern by forcing a re-save with UTF8NoBOM encoding.

**Usage:**
```powershell
# Remove BOM from all .tmdl files in semantic model
.\scripts\remove_all_boms.ps1 -RootPath "IAM_FIELD_MAPPING.SemanticModel\definition" -Extension "*.tmdl"

# Remove BOM from all .json files in report definition
.\scripts\remove_all_boms.ps1 -RootPath "IAM Portfolio Surverillance.Report\definition" -Extension "*.json"

# Remove BOM from .tmdl files in a specific folder
.\scripts\remove_all_boms.ps1 -RootPath "IAM_FIELD_MAPPING.SemanticModel\definition\tables" -Extension "*.tmdl"
```

**Parameters:**
- `-RootPath` (required): Root directory to scan for files
- `-Extension` (optional, default: `*.json`): File extension pattern to filter (e.g., `*.tmdl`, `*.json`, `*`)

**How it works:**
- Reads file content as a raw string
- Forces re-write using `System.Text.UTF8Encoding($false)` (UTF-8 without BOM)
- Reports success for each file processed

#### Convenience Wrapper: `cleanup_after_edit.ps1`

Cleans up both semantic model and report files in one command.

**Usage:**
```powershell
# Clean both semantic model and report definition
.\scripts\cleanup_after_edit.ps1 -CleanBoth

# Clean semantic model only
.\scripts\cleanup_after_edit.ps1 -SemanticModelPath "IAM_FIELD_MAPPING.SemanticModel\definition"

# Clean report definition only
.\scripts\cleanup_after_edit.ps1 -ReportPath "IAM Portfolio Surverillance.Report\definition"
```

#### Python: `remove_bom.py`

Cross-platform script to remove UTF-8 BOM from files (useful for non-Windows systems).

**Usage:**
```bash
# Remove BOM from all .tmdl files
python scripts/remove_bom.py "IAM_FIELD_MAPPING.SemanticModel/definition" --extension "*.tmdl" --verbose

# Remove BOM from all .json files in report
python scripts/remove_bom.py "IAM Portfolio Surverillance.Report/definition" --extension "*.json"
```

**Parameters:**
- `root_path` (required positional): Root directory to scan
- `--extension` (optional, default: `*.json`): File extension pattern to filter
- `--verbose` (optional flag): Print details for every file checked

### Recommended Workflow

After any TMDL or report JSON edits:

1. Make your edits to `.tmdl` or `.json` files
2. Verify changes are correct
3. Run BOM removal on the affected folder using one of these approaches:
   
   **Option A: Clean semantic model only**
   ```powershell
   .\scripts\remove_all_boms.ps1 -RootPath "IAM_FIELD_MAPPING.SemanticModel\definition" -Extension "*.tmdl"
   ```
   
   **Option B: Clean report definition only**
   ```powershell
   .\scripts\remove_all_boms.ps1 -RootPath "IAM Portfolio Surverillance.Report\definition" -Extension "*.json"
   ```
   
   **Option C: Clean both (convenience wrapper)**
   ```powershell
   .\scripts\cleanup_after_edit.ps1 -CleanBoth
   ```

4. Verify output shows `✓ Fixed` for the files you edited
5. Commit changes with cleaned encoding

### If BOM Errors Persist

If Power BI or Fabric still report BOM encoding errors after running the cleanup script:

1. Run the cleanup script again with verbose output to confirm files were processed
2. Verify the file is actually being read from the correct location
3. Try a manual force-resave:
   ```powershell
   $content = Get-Content -Raw -Path "path\to\file.tmdl"
   [System.IO.File]::WriteAllText("path\to\file.tmdl", $content, (New-Object System.Text.UTF8Encoding($false)))
   ```
4. Reload the model in Power BI Desktop or redeploy to Fabric
...
- **`scripts/remove_bom.py`** - Generic Python script to remove UTF-8 BOM from files (cross-platform)
- **`scripts/remove_all_boms.ps1`** - Generic PowerShell script to remove UTF-8 BOM from files (Windows)
