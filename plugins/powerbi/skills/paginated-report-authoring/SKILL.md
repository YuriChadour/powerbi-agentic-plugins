...
description: >
  Create, generate, validate, parameterize, and publish Fabric paginated reports (.rdl)
  bound to Power BI semantic models using DAX datasets and the Power BI Imports API.
  Triggers: create paginated report, RDL report, report builder RDL, publish RDL,
  upload RDL to Fabric, paginated report parameters, paginated report DAX,
  InvalidDefinitionFormat, or paginated-report troubleshooting.
  Does not handle interactive PBIR/PBIP pages, visuals, themes, or bookmarks; use
  `powerbi-report-authoring` for those requests.
---

> Source attribution: adapted from Microsoft `skills-for-fabric` PR #65 at commit
> `1e42d58` (MIT-licensed repository), with repository-specific routing, paths, and
> safety adaptations.
...
# paginated-report-authoring — Paginated Report Authoring over a Semantic Model via CLI
...
|---|---|---|
| Finding Workspaces and Items in Fabric | [COMMON-CLI.md § Finding Workspaces and Items in Fabric](../../../../common/COMMON-CLI.md#finding-workspaces-and-items-in-fabric) | **Mandatory** — workspace/model ID resolution |
...
- `nameConflict=Overwrite` unless an existing report is confirmed; use the publisher's explicit overwrite switch only for re-publication.
...
- Pair with `semantic-model-authoring` or `fabric-cli` (schema discovery) and `powerbi-report-management`
  (post-publish governance/permissions) where available.

> The common references above resolve from this repository's root `common/` directory. The
> topic-specific `references/` files remain the authoritative implementation guidance.
