#!/usr/bin/env python3
"""Generate a semantic-model-bound paginated report RDL."""
import argparse
import base64
import json
import os
...
def stable_id(label):
    """Create reproducible RDL identifiers without embedding environment IDs."""
    return str(uuid.uuid5(uuid.NAMESPACE_URL, f"paginated-report-authoring:{label}:{DATASET_GUID}"))
...
<Report MustUnderstand="df" xmlns="{NS}" xmlns:rd="{RD}" xmlns:df="{NS}/defaultfontfamily">
  ...
  <rd:ReportID>{stable_id('report')}</rd:ReportID>
  ...
  <DataSources>
    <DataSource Name="PowerBIDataset">
      <ConnectionProperties>
        ...
        <ConnectString>Data Source=pbiazure://api.powerbi.com/;Identity Provider="https://login.microsoftonline.com/{TENANT_ID}, https://analysis.windows.net/powerbi/api";Initial Catalog=sobe_wowvirtualserver-{DATASET_GUID};Integrated Security=ClaimsToken</ConnectString>
      </ConnectionProperties>
      <rd:SecurityType>None</rd:SecurityType>
      <rd:DataSourceID>{stable_id('datasource')}</rd:DataSourceID>
      <rd:PowerBIWorkspaceName>{esc(WORKSPACE)}</rd:PowerBIWorkspaceName>
...
def main():
    ...
    parser.add_argument("--dataset-guid", required=True, help="Semantic model GUID used in the RDL connection string")
    parser.add_argument("--dataset-name", default="semantic_model", help="Semantic model display name used in report text")
    parser.add_argument("--workspace-name", default="workspace", help="Workspace display name used in RDL designer metadata")
    parser.add_argument("--tenant-id", default="organizations", help="Tenant ID or organizations for the identity provider")
    parser.add_argument("--display-name", default="paginated_report", help="Report display name")
    parser.add_argument("--output", default="paginated_report.rdl", help="Output RDL path")
    parser.add_argument("--request-output", help="Optional output path for the definition request JSON")
    args = parser.parse_args()

    global DATASET, DATASET_GUID, WORKSPACE, TENANT_ID
    ...
    rdl_path = Path(args.output).resolve()
    rdl_path.parent.mkdir(parents=True, exist_ok=True)
    with rdl_path.open("w", encoding="utf-8") as f:
        f.write(rdl)
    payload = base64.b64encode(rdl.encode("utf-8")).decode("ascii")
    display_name = args.display_name
    file_name = rdl_path.name
    req = {
        "displayName": display_name,
        "description": f"Paginated report over the {DATASET} semantic model.",
        "definition": {
            "format": "PaginatedReportDefinition",
            "parts": [
                {"path": file_name, "payload": payload, "payloadType": "InlineBase64"}
            ]
        }
    }
    req_path = Path(args.request_output).resolve() if args.request_output else rdl_path.with_name(f"{rdl_path.stem}_request.json")
