...
The Fabric plugin MCP configuration will add a `FabricIQ` server alongside the existing `fabric-mcp-server`, using `https://fabriciq.svc.cloud.microsoft/v1/mcp/fabriciq` with all server-provided tools enabled. The implementation will verify that the endpoint exposes the required read/query operations before adding routing redirects. This prevents the broken-routing problem in the reference report-management text.
