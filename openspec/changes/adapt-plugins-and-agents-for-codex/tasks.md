...
- [x] 1.2 Inventory `.claude-plugin/marketplace.json`, every `plugins/*` root, all `skills/**/SKILL.md`, all top-level plugin agent Markdown files, all checked-in Codex agent adapters, and all `.mcp.json` files; add machine-checkable source-to-projection catalog validation that fails on missing, stale, orphaned, or undeclared adapter entries
- [x] 1.3 Define the target-specific projection layout, checked-in adapter mapping, ownership markers, same-name MCP conflict policy, and non-force behavior; verify neither target overlaps the other target's configuration or unrelated user assets
...
- [x] 2.6 Add a checked-in Codex `.toml` adapter beside every packaged top-level agent Markdown file; verify parity/reference tests preserve each role's source instructions, required skills, safety constraints, workflow expectations, and identity mapping
...
- [x] 3.2 Implement target-specific destination resolution and projection of selected shared assets, copying checked-in Codex `.toml` adapters without Markdown-to-TOML conversion; verify default `All`, `Codex`, `Copilot`, and single-plugin installs in isolated temporary user profiles
...
- [x] 3.5 Add capability- and target-scoped checks/provisioning for Node/npm, `uv`, Power BI Desktop Bridge, ADOMD.NET, VS Code's Python extension, and other selected-plugin dependencies; verify existing capabilities are reported and skipped even with `-Force`, optional failures are warnings, and required failures are actionable target errors
...
- [x] 4.1 Add static tests that enumerate all five plugins, every packaged agent Markdown file, every checked-in Codex adapter, every skill, each MCP definition, and every source-to-projection mapping; verify no catalog item is omitted, adapter pairs remain in parity, and all references resolve
- [x] 4.2 Add integration tests for default `All`, explicit `All`, `Codex`, `Copilot`, selected-plugin, force update, non-force rerun, invalid target/plugin, missing repository, missing/stale adapter, backup/recovery, missing optional dependency, and MCP conflict/failure cases; verify exit codes and filesystem/configuration effects and prove setup never synthesizes an adapter
...
- [x] 5.4 Add a compatibility/support matrix and maintainer guidance for adding plugins, agents, and checked-in host-specific adapters; verify each new catalog entry has a source-to-projection validation path and every new top-level agent Markdown file includes its Codex `.toml` pair before release
