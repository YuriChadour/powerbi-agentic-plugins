...
- Add a `dax-unit-testing` skill to `plugins/powerbi/skills/` staging the PQL.Assert assertion library (`functions.tmdl`), model-independence and reserved-word references, the `Certification/MeasureCertification.csv` registry schema (columns, `TestCategory`/`Status`/`ApprovalSource` lifecycle, `FilterExpression` rules, integrity checklist), and registry automation scripts (`validate_registry.py`, `generate_measure_tests.py`, `certify_measures.py`, `coverage_report.py`).
