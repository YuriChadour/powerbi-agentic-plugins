...
When a spec's first measure task targets a semantic model that does not yet have the PQL.Assert assertion library deployed, the `powerbi-architect` agent SHALL add a single one-time setup task to deploy the library before the first `sync` task, rather than checking for installation in every subsequent measure task.
