...
`setup-team-plugins.ps1` defaults to `-Target All`. Use `-Target Codex` for `%USERPROFILE%\.codex` only or `-Target Copilot` for the existing `.copilot` workflow. Use `-PluginName` for one of `powerbi`, `fabric`, `devops`, `skill-creator`, or `spec-lifecycle`; use `-Force` for an installer-owned update. Codex MCP entries are written to `.codex\config.toml` from the plugin `.mcp.json` files, while Copilot configuration remains isolated. Backups are retained under each target's own backup directory.
